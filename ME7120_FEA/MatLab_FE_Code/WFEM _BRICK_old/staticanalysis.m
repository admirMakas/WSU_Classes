function X=staticanalysis
% STATICANALYSIS solves the static response problem and outputs the
% results to a variable X.  
...
lslave=length(slavedofs);
  if lslave>0% Here we need to reduce out constrained DOFs before
             % eigen analysis. Slave DOFs are defined when we apply
    ...
    i=sort(i); % sort them. This puts the slave coordinates all up
               % top. 
    master=i(lslave+1:size(nodes,1)*3);%The master coordinates are
